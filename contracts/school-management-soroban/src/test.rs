...
//     let env = Env::default();
//     let contract_id = env.register(SchoolManagement, ());
//     let client = SchoolManagementClient::new(&env, &contract_id);

//     let student_wallet = Address::random(&env);
//     let name = String::from_str(&env, "james");
//     let class_name = Class::One;

//     let student_registration = client.register_student(&student_wallet, &name, &class_name);
//     assert_eq!(student_registration.student_wallet, student_wallet);
//     assert_eq!(student_registration.name, name);
//     assert_eq!(student_registration.class_name, class_name);
// }
...
use crate::school_management::{SchoolManagement, SchoolManagementClient}; 
use crate::storage::Class;
...
fn test_update_student_class() {
    ...
    setup_result.client.register_student(&setup_result.student_wallet, &name, &Class::College);
    ...
    match student.class_name {
        Class::HighSchool => assert!(true),
        _=> panic!("class was not updated"),
    }
}
...
fn test_get_student_payment_history() {
    ...
    setup_result.token_client.mint(&setup_result.student_wallet, &amount);

    setup_result.client.make_payment(&student_id, &amount);

    let payments = setup_result.client.get_student_payment_history(&student_id);
